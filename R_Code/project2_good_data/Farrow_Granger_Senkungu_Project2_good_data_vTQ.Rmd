...
mylda<- lda(diagnosis ~ texture+perimeter+area+smoothness+compactness+concavity+concave_points+symmetry+fractal_dimension, data = wdbc_sub)
myqda<- qda(diagnosis ~ texture+Cell_Size+Cell_Shape+Adhesion+Epithelial+Nuclei+Chromatin+Nucleoli+Mitoses, data = wdbc_sub)
...
pred <- prediction(ldaprd, wdbc_sub$Class)
...
full.model <- lm (Class ~., data = wdbc_sub)
```

## LDA

```{r LDA}
wdbc_sub.continuous <- wdbc_sub[, c(2:11)]
pairs(wdbc_sub.continuous[,1:10])
